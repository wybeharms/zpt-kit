...
- **Look freely; change only with a plan and a yes.** Reading is never gated: sweeping, scanning, and reading files need no permission and no pause; announce in one line what is being looked at and keep going. Writes are the opposite: nothing is created, moved, renamed, or deleted until the person has seen a short plan of action in the current chat and said yes, for every batch of changes, not only at the big Propose gate. The only earlier writes are `session-notes.md`, the plugin-mode marker line (announced in one line when written), and anything the person explicitly approves in the moment (the starter global file).
...
2. **The global layer.** The tour's silent check confirmed the headline facts; now look properly at the product's global folder (`~/.claude/` on Claude, `~/.codex/` on ChatGPT/Codex; see `key-concepts.md`, "Two layers" and "The hidden global folder") and report what is there: the global instructions file, `skills/`, `memory/`, configuration. Recap the global file in five bullets max.
   ...
   If none exists: say so in one line and propose a starter global file, shown as a short ready-to-approve list: their name plus the recommended conventions from "Conventions worth setting once" (`key-concepts.md`). Create it only on an explicit yes; otherwise record it as a finding for Execute. If one exists: after the confirmation above, silently compare it against "Conventions worth setting once" and carry any missing conventions worth having into the Propose plan. Either way, the journey never ends with the conventions merely mentioned: they get proposed concretely and written on approval.
...
3. **The sweep.** Run it right away, without asking permission: reading changes nothing, so announce in one line that a read-only look around is starting and go. Begin with the folder this session is opened in and understand its layout, then search likely roots (Documents, Desktop, home, cloud-storage folders) for instructions files (`CLAUDE.md`, `AGENTS.md`, stray `GEMINI.md`), `.claude/skills/` folders, and stray `session-notes.md` files from an unfinished ZPT Kit journey, and cross-check `~/.claude/projects/` (its subfolder names encode every folder ever mounted; see `key-concepts.md`). Ask where the person's folders and files live too, but never wait on the answer to start reading: someone with sprawl has lost track by definition. If Cowork is in the picture, hunt down Cowork's own folder: half the work may be sitting there. **Check each found folder for sharing signals.** A folder connected to GitHub (a `.git` folder inside; `git remote -v` names the host) may be shared, with other people seeing the same files. A folder whose name starts with `zpt` is a working folder built and maintained by ZPT Partners for a client team, shared through private GitHub or SharePoint. Never treat a shared folder as the person's private property: read its instructions file to see how it runs, say plainly that it is shared, and never propose restructuring it solo. Merge what the sweep finds with what the person named, leave the kit files and everything under the product's plugin directory out of the inventory, and play the merged inventory back in plain language, shared folders labeled as such, reading deeper as needed without further permission.
...
- 📁 Print the proposed **target tree**: the folder(s), the instructions file (`CLAUDE.md` on Claude, `AGENTS.md` on ChatGPT/Codex), a short system map (a section inside the instructions file, or its own `system-map.md` once it outgrows that; never an invented name), the sub-folders, the first guideline files. One plain line of explanation per item. When improving, show it against the current-state tree so the person sees before and after.
...
- a short **system map**: a table of contents for the folder. It lives as the "System map" section of the instructions file, and graduates to its own `system-map.md` at the folder root only when it outgrows that section; always this name, never an invented one ("where things live", "folder overview"),
...
- **Conventions in place.** Before the handoff, make sure the global instructions file actually carries the agreed conventions from "Conventions worth setting once" (`key-concepts.md`): propose any missing ones as a ready-to-approve list, write them on approval, and keep local files free of duplicates.
